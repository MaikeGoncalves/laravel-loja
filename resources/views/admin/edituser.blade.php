...
@section('content')
    ...
    <div class="w-full p-6 font-semibold text-2xl text-gray-600 flex items-center">
        <ion-icon name="person-add-sharp" class="mr-2"></ion-icon>Editar #{{ $user->id }} {{ $user->name }}
    </div>

    <div class="px-6">
        <form action="{{ route('user.editAction', $user->id) }}" method="POST" enctype="multipart/form-data" class="">
            @method('PUT')
            @csrf
            {{--nome--}}
